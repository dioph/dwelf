...
def eker(self, star, spot):
    ...
    if ng >= 1:
        ...
        k4_g = - (sinphi0_g - sinphi0_g ** 3 / 3) * ((3.0 / 8) * (rad - rad0_g) + (1.0 / 16)
                                                     * (np.sin(2 * rad) * (np.cos(2 * rad) - 4) - np.sin(2 * rad0_g) * (
                        np.cos(2 * rad0_g) - 4)))
    ...
    if nc >= 1:
        ...
        k4_c = (sinphi0_c - sinphi0_c ** 3 / 3) * ((3.0 / 8) * (rad - rad0_c) + (1.0 / 16)
                                                   * (np.sin(2 * rad) * (np.cos(2 * rad) - 4) - np.sin(2 * rad0_c) * (
                        np.cos(2 * rad0_c) - 4)))
    ...
    lc = 1.0 + (iratio - 1.0) / (np.pi * (1.0 - limb1 / 3.0 + limb2 / 6.0)) * (
...
class Modeler(object):
    def __init__(self, l1=0.68, l2=0.00, ir=0.22, x=np.linspace(0, 45, 500), y=np.ones(500), rmin=0.5, rmax=1.5,
        ...
        self.y = y  # flux array
        self.rmin = rmin
        self.rmax = rmax  # radius (in solar units)
        self.inc_min = inc_min
        self.inc_max = inc_max  # stellar inclination
        self.Teq_min = Teq_min
        self.Teq_max = Teq_max  # period at equator
        self.k_min = k_min
        self.k_max = k_max  # differential rotation
        self.lat_min = lat_min
        self.lat_max = lat_max  # spot latitude
        self.lon_min = lon_min
        self.lon_max = lon_max  # spot longitude
        self.rad_min = rad_min
        self.rad_max = rad_max  # spot radius (in degrees)
        ...
        self.v_min = v_min
        self.v_max = v_max  # v sin i (in km/s)
    ...
    def eker(self, theta):
        ...
        if ng >= 1:
            ...
            k1_g = ((np.pi - phi0_g) / 4) * (cosrad0_g ** 4 - cosrad ** 4)
            k2_g = (sinphi0_g / 8) * (rad0_g - rad + 0.5 * (np.sin(2 * rad)
                                                            * np.cos(2 * rad) - np.sin(2 * rad0_g) * np.cos(
                        2 * rad0_g)))
            k3_g = (1.0 / 8) * (np.pi - phi0_g - sinphi0_g *
                                cosphi0_g) * (sinrad ** 4 - sinrad0_g ** 4)
            k4_g = - (sinphi0_g - sinphi0_g ** 3 / 3) * ((3.0 / 8) * (rad - rad0_g) + (1.0 / 16)
                                                         * (np.sin(2 * rad) * (np.cos(2 * rad) - 4) - np.sin(
                        2 * rad0_g) * (np.cos(2 * rad0_g) - 4)))
            cl_g = ((np.pi - phi0_g) / 3) * (cosrad ** 3 - cosrad0_g ** 3) * (1 - 3 * costhe0_g ** 2) - (
                    np.pi - phi0_g - sinphi0_g * cosphi0_g) * (cosrad - cosrad0_g) * sinthe0_g ** 2 - \
                   (4.0 / 3) * sinphi0_g * (sinrad ** 3 - sinrad0_g ** 3) * sinthe0_g * costhe0_g - \
                   (1.0 / 3) * sinphi0_g * cosphi0_g * \
        ...
        if nc >= 1:
            ...
            k1_c = (phi0_c / 4) * (cosrad0_c ** 4 - cosrad ** 4)
            k2_c = - (sinphi0_c / 8) * (rad0_c - rad + 0.5 * (np.sin(2 * rad)
                                                              * np.cos(2 * rad) - np.sin(2 * rad0_c) * np.cos(
                        2 * rad0_c)))
            k3_c = (1.0 / 8) * (phi0_c + sinphi0_c * cosphi0_c) * \
                   (sinrad ** 4 - sinrad0_c ** 4)
            k4_c = (sinphi0_c - sinphi0_c ** 3 / 3) * ((3.0 / 8) * (rad - rad0_c) + (1.0 / 16)
                                                       * (np.sin(2 * rad) * (np.cos(2 * rad) - 4) - np.sin(
                        2 * rad0_c) * (np.cos(2 * rad0_c) - 4)))
            cq_c = 2 * costhe0_c ** 3 * k1_c + 6 * costhe0_c ** 2 * sinthe0_c * \
                   k2_c + 6 * costhe0_c * sinthe0_c ** 2 * k3_c + 2 * sinthe0_c ** 3 * k4_c
            ic[jc] = phi0_c * costhe0_c * sinrad ** 2 - \
                     np.arcsin(cosrad / sinthe0_c) - 0.5 * sinthe0_c * \
    ...
    def solve(self, theta):
        ...
        for i in range(1, nspots + 1):
            y += -1 + self.eker(np.append(theta[:3], theta[3 * i:3 * (i + 1)]))
        return y
    ...
    def normalize(self, n_bins=500):
        ...
        good = ~np.isnan(self.y)  # logical_and(~isnan(self.y), self.x <= 45)  # removed NaNs
        self.x = self.x[good]
        self.y = self.y[good]
    ...
    def vsini(self, i, T):
        ...
        return [50.592731692185623 * self.rmin * np.sin(i * np.pi / 180) / T,
                50.592731692185623 * self.rmax * np.sin(i * np.pi / 180) / T]
    ...
    def lnprior(self, theta):
        ...
        if self.v_max < v[0] or v[1] < self.v_min:
            return -np.inf  # restrict v sin i
        if not (
                self.inc_min < inc < self.inc_max and self.Teq_min < Teq < self.Teq_max and self.k_min < k < self.k_max):
            return -np.inf  # restrict star params
        for i in range(1, nspots + 1):
            if not (self.lat_min < theta[3 * i] < self.lat_max and self.lon_min < theta[3 * i + 1]
                ...
                return -np.inf  # restrict spot params
        return 0.0
    ...
    def llsq(self, p0s, n_iter=0, star_params=None):
        ...
        for p0 in p0s:
            ...
            fps, = leastsq(self.eps, p1, args=star_params, maxfev=n_iter)
